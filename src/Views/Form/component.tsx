...
export default function Form() {
    ...
    const checkPassword = (repeatedPassword:string) => {
        password === repeatedPassword ? setPasswordCheck(true) : setPasswordCheck(false)
    }
    
    const handleSubmit = () => {
        setPayment(true)
        let customer:Customer = {name, surname, sex, birthdate, birthplace, city, country, address, cap, telephone, email, username, password}
    ...
    return <section className={classes.formContainer}>
        ...
        <form className={classes.formContainer__form}>
            ...
            <select name='sex' className={classes.formContainer__form__module} onChange={(e) => setSex(e.target.value)}>
            ...
            <hr />
            <input type="text" placeholder="Inserire l'indirizzo email" name='email' className={classes.formContainer__form__module} onChange={(e) => setEmail(e.target.value)}/>
            <input type="text" placeholder="Inserire il nome utente" name='username' className={classes.formContainer__form__module} onChange={(e) => setUsername(e.target.value)}/>
            <div className={classes.formContainer__form__passwordContainer}>
                <input type={passwordVisibile ? 'text' : 'password'} placeholder="Inserire la password" name='password' className={classes.formContainer__form__passwordContainer__module} onChange={(e) => setPassword(e.target.value)}/>
                { passwordVisibile
                    ? <button className={classes.formContainer__form__passwordContainer__visibilityButton} onClick={(e) => {e.preventDefault(); changePasswordVisibility()}}>
            ...
            <div className={classes.formContainer__form__passwordContainer}>
                <input type={passwordVisibile ? 'text' : 'password'} placeholder="Ripetere password" name='passwordRepeat' className={classes.formContainer__form__passwordContainer__module} onChange={(e) => checkPassword(e.target.value)}/>
                { passwordVisibile
                    ? <button className={classes.formContainer__form__passwordContainer__visibilityButton} onClick={changePasswordVisibility}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
                            <path d="M150.7 92.77C195 58.27 251.8 32 320 32C400.8 32 465.5 68.84 512.6 112.6C559.4 156 590.7 207.1 605.5 243.7C608.8 251.6 608.8 260.4 605.5 268.3C592.1 300.6 565.2 346.1 525.6 386.7L630.8 469.1C641.2 477.3 643.1 492.4 634.9 502.8C626.7 513.2 611.6 515.1 601.2 506.9L9.196 42.89C-1.236 34.71-3.065 19.63 5.112 9.196C13.29-1.236 28.37-3.065 38.81 5.112L150.7 92.77zM189.8 123.5L235.8 159.5C258.3 139.9 287.8 128 320 128C390.7 128 448 185.3 448 256C448 277.2 442.9 297.1 433.8 314.7L487.6 356.9C521.1 322.8 545.9 283.1 558.6 256C544.1 225.1 518.4 183.5 479.9 147.7C438.8 109.6 385.2 79.1 320 79.1C269.5 79.1 225.1 97.73 189.8 123.5L189.8 123.5zM394.9 284.2C398.2 275.4 400 265.9 400 255.1C400 211.8 364.2 175.1 320 175.1C319.3 175.1 318.7 176 317.1 176C319.3 181.1 320 186.5 320 191.1C320 202.2 317.6 211.8 313.4 220.3L394.9 284.2zM404.3 414.5L446.2 447.5C409.9 467.1 367.8 480 320 480C239.2 480 174.5 443.2 127.4 399.4C80.62 355.1 49.34 304 34.46 268.3C31.18 260.4 31.18 251.6 34.46 243.7C44 220.8 60.29 191.2 83.09 161.5L120.8 191.2C102.1 214.5 89.76 237.6 81.45 255.1C95.02 286 121.6 328.5 160.1 364.3C201.2 402.4 254.8 432 320 432C350.7 432 378.8 425.4 404.3 414.5H404.3zM192 255.1C192 253.1 192.1 250.3 192.3 247.5L248.4 291.7C258.9 312.8 278.5 328.6 302 333.1L358.2 378.2C346.1 381.1 333.3 384 319.1 384C249.3 384 191.1 326.7 191.1 255.1H192z"/>
                        </svg>
                    </button>
